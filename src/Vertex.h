...
struct Vertex
{
    Vertex(float vz, float sigma2, vector<float> hitx1, vector<float> hity1, vector<float> hitz1, vector<float> hitx2, vector<float> hity2, vector<float> hitz2, vector<float> candz) : vz(vz), sigma2(sigma2), hitx1(hitx1), hity1(hity1), hitz1(hitz1), hitx2(hitx2), hity2(hity2), hitz2(hitz2), candz(candz){};
...
void Draw_RecoVtxTklXY(TH2F *hM_clusXY_half1, TH2F *hM_clusXY_half2, vector<TLine *> vtl_half1, vector<TLine *> vtl_half2, float PVx_half1, float PVx_half2, float PVx, float PVy_half1, float PVy_half2, float PVy, int dZbin, bool isdata, TString plotname_tstr)
{
    SetsPhenixStyle();
    gStyle->SetPalette(kThermometer);
...
void Draw_1DHistVtxDemo(TH1F *hM_half1, TH1F *hM_half2, const char *Xtitle, const char *Ytitleunit, int ndivision, bool isdata, bool ispreliminary, const char *date, TString plotname_tstr)
{
    SetsPhenixStyle();
    // Merge two histograms
    TH2F *hM_merge = (TH2F *)hM_half1->Clone();
    hM_merge->Add(hM_half2);
    // Get bin size of the histogram
    double binwidth = hM_merge->GetBinWidth(1);
    ...
    gPad->SetLeftMargin(0.15);
    gPad->SetBottomMargin(0.13);
    hM_merge->GetXaxis()->SetTitle(Xtitle);
    hM_merge->GetYaxis()->SetTitle(Form("Counts / %g %s", binwidth, Ytitleunit));
    hM_merge->GetXaxis()->SetTitleOffset(1.2);
    hM_merge->GetYaxis()->SetTitleOffset(1.55);
    hM_merge->GetYaxis()->SetRangeUser(0, hM_merge->GetMaximum() * 1.35);
    hM_merge->GetXaxis()->SetNdivisions(ndivision, kTRUE);
    hM_merge->SetLineColor(1);
    hM_merge->SetLineWidth(2);
    hM_merge->Draw("hist");
    TLatex *ltx = new TLatex();
    ltx->SetNDC();
    ltx->SetTextSize(0.045);
...
void Draw_2DHistVtxDemo(TH2F *hM_half1, TH2F *hM_half2, const char *Xtitle, const char *Ytitle, int ndivision, bool isdata, bool ispreliminary, const char *date, TString plotname_tstr)
{
    SetsPhenixStyle();
    gStyle->SetPalette(kThermometer);
    // Merge two histograms
    ...
    gPad->SetBottomMargin(0.13);
    hM_merge->GetXaxis()->SetTitle(Xtitle);
    hM_merge->GetYaxis()->SetTitle(Ytitle);
    hM_merge->GetXaxis()->SetTitleOffset(1.2);
    hM_merge->GetYaxis()->SetTitleOffset(1.45);
    hM_merge->GetXaxis()->SetNdivisions(ndivision, kTRUE);
    hM_merge->GetYaxis()->SetNdivisions(ndivision, kTRUE);
    hM_merge->Draw("colz");
    TLatex *ltx = new TLatex();
    ltx->SetNDC();
    ltx->SetTextSize(0.045);
...
tuple<vector<float>, int, TH2F *, TH2F *, vector<TLine *>, vector<TLine *>, TH1F *, TH2F *, TH1F *, TH2F *, TH1F *, TH2F *, TH2F *> Tracklet3DPV(vector<Hit *> layer1, vector<Hit *> layer2, int dPhiCutbin, int dZCutbin)
{
    // get a random number of integer
    TRandom3 *r = new TRandom3(0);
    int random_int = r->Integer(1E9);
    // tuples for (1) vector of PV estimates, (2) number of candidates in the vertex cluster, (3) TH2F of cluster positions in Z-Rho, (4) TH2F of cluster positions in X-Y, (5) vector of TLines in Z-Rho, (6) vector of TLines in X-Y, (7) TH1F of difference between extrapolated z and PV z, (8) TH2F of difference between extrapolated x/y and PV x/y,
    // (9) TH1F of z position of vertex candidates, (10) TH2F of x/y position of vertex candidates
    ...
    TH1F *hM_vtxcandz = new TH1F(Form("hM_vtxcandz_%d", random_int), Form("hM_vtxcandz_%d", random_int), 400, -20, 20);
    TH2F *hM_vtxcandxy = new TH2F(Form("hM_vtxcandxy_%d", random_int), Form("hM_vtxcandxy_%d", random_int), 100, -2, 2, 100, -2, 2);
