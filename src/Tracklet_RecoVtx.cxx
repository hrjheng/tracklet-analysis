...
int main(int argc, char *argv[])
{
    ...
    // for (Long64_t ev = 0; ev < t->GetEntriesFast(); ev++)
    {
        ...
        Draw_1DHistVtxDemo(hM_diffExptzPVz_halves1,hM_diffExptzPVz_halves2, "v_{Z}^{Candidate} - PV_{Z}^{Estimate} [cm]", "cm", 510, IsData, true, "2023-08-30", Form("%s_Evt%d_diffExptzPVz", demoplotname.Data(), event));
        Draw_2DHistVtxDemo(hM_diffExptxyPVxy_halves1, hM_diffExptxyPVxy_halves2, "v_{X}^{Candidate} - PV_{X}^{Estimate} [cm]", "v_{Y}^{Candidate} - PV_{Y}^{Estimate} [cm]", 510, IsData, true, "2023-08-30", Form("%s_Evt%d_diffExptxyPVxy", demoplotname.Data(), event));

        Draw_1DHistVtxDemo(hM_VtxCandZ_halves1, hM_VtxCandZ_halves2, "v_{Z}^{Candidate} [cm]", "cm", 510, IsData, true, "2023-08-30", Form("%s_Evt%d_VtxCandZall", demoplotname.Data(), event));
        Draw_2DHistVtxDemo(hM_VtxCandXY_halves1, hM_VtxCandXY_halves2, "v_{X}^{Candidate} [cm]", "v_{Y}^{Candidate} [cm]", 510, IsData, true, "2023-08-30", Form("%s_Evt%d_VtxCandXYcluster", demoplotname.Data(), event));

        Draw_1DHistVtxDemo(hM_DCAxy_VtxClus_halves1, hM_DCAxy_VtxClus_halves2, "DCA_{xy} [cm]", "cm", 505, IsData, false, "", Form("%s_Evt%d_DCAxy", demoplotname.Data(), event));
        Draw_2DHistVtxDemo(hM_DCAxy_DCAz_VtxClus_halves1, hM_DCAxy_DCAz_VtxClus_halves2, "DCA_{xy} [cm]", "DCA_{z} [cm]", 505, IsData, false, "", Form("%s_Evt%d_DCAxyDCAz", demoplotname.Data(), event));
        Draw_2DHistVtxDemo(hM_DCAx_DCAy_VtxClus_halves1, hM_DCAx_DCAy_VtxClus_halves2, "DCA_{x} [cm]", "DCA_{y} [cm]", 505, IsData, false, "", Form("%s_Evt%d_DCAxDCAy", demoplotname.Data(), event));
