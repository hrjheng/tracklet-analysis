...
void calccorr(int layer = 12, int CentBinLow = -1, int CentBinHigh = 20, bool applyc = false, bool applyg = false, bool applym = false, TString estag = "", TString putag = "", bool debug = false)
{
    // Options
    // int layer = TString(argv[1]).Atoi();
    // int CentBinLow = TString(argv[2]).Atoi();
    // int CentBinHigh = TString(argv[3]).Atoi();
    ...
    system(Form("mkdir -p ./plot/corrections/layer%d_CentBin%dto%d", layer, (CentBinLow == CentBinHigh) ? CentBinLow : CentBinLow + 1, (CentBinLow == CentBinHigh) ? CentBinHigh : CentBinHigh - 1));

    if (layer != 12 && layer != 23 && layer != 13)
    {
        cout << "[ERROR] layer " << layer << " is not one of (12,23,13). Abort." << endl;
        return;
    }

    TString infilenam = Form("/sphenix/user/hjheng/TrackletAna/minitree/AuAu_Nominal_NoPileup/TrackletAna_minitree_layer%d_Evt0to2000_RandhitCase0_MisAlignNum0.root", layer);
    TFile *finput = new TFile(infilenam, "read");
    TTree *tinput = (TTree *)finput->Get(Form("minitree_%d", layer));
    ...
    if (applyc)
    {
        fcorr = new TFile(Form("./plot/corrections/layer%d_CentBin%dto%d/correction_hists.root", layer, (CentBinLow == CentBinHigh) ? CentBinLow : CentBinLow + 1, (CentBinLow == CentBinHigh) ? CentBinHigh : CentBinHigh - 1));
    ...
    TCut csel;
    if (CentBinLow == CentBinHigh)
    {
        csel = Form("CentralityBin==%d", CentBinLow);
    }
    else
    {
        csel = Form("CentralityBin>%d && CentralityBin<%d", CentBinLow, CentBinHigh);
    }
    TCut vsel = "PV_z<=10 && PV_z>=-10";
    TCut osel = "trig";
    TCut psel = "process==0";
    ...
    //     hgaccep->Draw("colz");
    //     cga->SaveAs(Form("./plot/corrections/layer%d_CentBin%dto%d/ga.png", layer, CentBinLow + 1, CentBinHigh - 1));
    // }
    ...
    ccheck->SaveAs(Form("./plot/corrections/layer%d_CentBin%dto%d/Check_Intermediate.pdf", layer, (CentBinLow == CentBinHigh) ? CentBinLow : CentBinLow + 1, (CentBinLow == CentBinHigh) ? CentBinHigh : CentBinHigh - 1));
    ...
    for (int x = 1; x <= neta; x++)
    {
        ...
        cfalphavz->SaveAs(Form("./plot/corrections/layer%d_CentBin%dto%d/alpha1Dfit-etabin%i.pdf", layer, (CentBinLow == CentBinHigh) ? CentBinLow : CentBinLow + 1, (CentBinLow == CentBinHigh) ? CentBinHigh : CentBinHigh - 1, x - 1));
    }
    ...
    for (int z = 1; z <= nvz; z++)
    {
        ...
        cfalphaeta->SaveAs(Form("./plot/corrections/layer%d_CentBin%dto%d/alpha1Dfit-vzbin%i.pdf", layer, (CentBinLow == CentBinHigh) ? CentBinLow : CentBinLow + 1, (CentBinLow == CentBinHigh) ? CentBinHigh : CentBinHigh - 1, z));
    }
    ...
    // h2alphafinal->GetZaxis()->SetRangeUser(h2alphafinal->GetMinimum(0), h2alphafinal->GetMaximum());
    ...
    calpha->SaveAs(Form("./plot/corrections/layer%d_CentBin%dto%d/alpha2D_inclTklMult.pdf", layer, (CentBinLow == CentBinHigh) ? CentBinLow : CentBinLow + 1, (CentBinLow == CentBinHigh) ? CentBinHigh : CentBinHigh - 1));
    ...
    for (int k = 0; k < nmult; k++)
    {
        ...
        calpha->SaveAs(Form("./plot/corrections/layer%d_CentBin%dto%d/alpha2D_TklMultb%d.pdf", layer, (CentBinLow == CentBinHigh) ? CentBinLow : CentBinLow + 1, (CentBinLow == CentBinHigh) ? CentBinHigh : CentBinHigh - 1, k));
    }
    ...
    hframe->GetYaxis()->SetRangeUser(0, ymax * 1.5);
    ...
    TLegend *l1 = new TLegend(0.13, 0.68, 0.5, 0.92);
    l1->SetHeader(Form("Closure test: MVTX layer %d&%d, CentBin %d to %d", ((int)layer / 10), (layer % 10), (CentBinLow == CentBinHigh) ? CentBinLow : CentBinLow + 1, (CentBinLow == CentBinHigh) ? CentBinHigh : CentBinHigh - 1), "l");
    ...
    l1->Draw();
    cstage->Draw();
    cstage->SaveAs(Form("./plot/corrections/layer%d_CentBin%dto%d/Closure.pdf", layer, (CentBinLow == CentBinHigh) ? CentBinLow : CentBinLow + 1, (CentBinLow == CentBinHigh) ? CentBinHigh : CentBinHigh - 1));

    /* Write histograms to file*/
    TFile *outf = new TFile(Form("./plot/corrections/layer%d_CentBin%dto%d/correction_hists.root", layer, (CentBinLow == CentBinHigh) ? CentBinLow : CentBinLow + 1, (CentBinLow == CentBinHigh) ? CentBinHigh : CentBinHigh - 1), "recreate");
    outf->cd();
    h2amapxev->Write();
    h1WGhadron->Write();
    h1WEraw->Write();
...
int main(int argc, char *argv[])
{
    ...
    // calccorr(int layer = 12, int CentBinLow = -1, int CentBinHigh = 20, bool applyc = false, bool applyg = false, bool applym = false, TString estag = "", TString putag = "", bool debug = false)

    // calccorr(12, -1, 20, false, false, false, "", "", false);
    vector<int> vl = {12, 23, 13};
    for (size_t i = 0; i < vl.size(); i++)
    {
        gStyle->SetPaintTextFormat();
        calccorr(vl[i], -1, 20, false, false, false, "", "", false);
        for (int cb = 0; cb < 20; cb++)
        {
            gStyle->SetPaintTextFormat();
            calccorr(vl[i], cb, cb, false, false, false, "", "", false);
        }
    }
